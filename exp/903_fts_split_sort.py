"""
FTS用のデータベースが分割されているので、それぞれについて検索を行い、最後にtf-idfの値に応じて結果をソートする
"""
...
def sort_join_by_tfidf(df: pd.DataFrame, fts_result: list[list[tuple[float, str]]]) -> list[str]:
    """
    tfidfの値によってソートして新しいcontextにする
    """
    context_list = []
    for ri, (_, row) in tqdm(enumerate(df.iterrows()), total=len(df)):
        text = row.prompt + " " + row.A + " " + row.B + " " + row.C + " " + row.D + " " + row.E
        vectorizer = TfidfVectorizer(analyzer="word", ngram_range=(1, 4), stop_words=stop_words)
        vectorizer.fit([text] + [r[1] for r in fts_result[ri]])
        # text と fts_result の類似度を計算
        text_vec = vectorizer.transform([text])
        fts_vec = vectorizer.transform([r[1] for r in fts_result[ri]])
        sim = cosine_similarity(text_vec, fts_vec)
        # fts_result をソート
        sorted_idx = np.argsort(sim)[0][::-1]
        sorted_fts_result = [fts_result[ri][idx] for idx in sorted_idx]
        # context を結合
        context = " ".join([r[1] for r in sorted_fts_result[:10]])[:5000]
        context_list.append(context)
    return context_list


def modify_prob_by_fts(
    df: pd.DataFrame, prob: np.ndarray, fts_num_max: int, model_path: str, fts_path: str
) -> tuple[pd.DataFrame, np.ndarray]:
    # cfg.fts_num の数だけ、np.max(pred2,axis=1) の値が小さいものから順に、FTSでcontextを修正し、予測し直して確率値を修正する

    max_prob = np.max(prob, axis=1)
    sorted_idx = np.argsort(max_prob)[:fts_num_max]
    fts_num = len(sorted_idx)

    # (rank, context)を格納するリストを各idxに対してFTSを6回やって作成し、それらをrankの値によってソートする
    fts_result = [[] for i in range(fts_num)]
    print(len(fts_result))

    db_list = glob(fts_path + "/*.db")
    ...
    for db_path in db_list:
        ...
        db.commit()
        for i, idx in tqdm(enumerate(sorted_idx), total=fts_num):
            row = df.iloc[idx]
            # FTSでcontextを修正
            res = search_fts(cur, row)
            fts_result[i].extend(res)
        db.close()

    """
    # rankの値によってソート
    for idx in range(fts_num):
        fts_result[idx].sort(key=lambda x: x[0])

    # context を結合
    context_list = ["".join([r[1] for r in res[:10]])[:5000] for res in fts_result]
    df.iloc[sorted_idx, df.columns.get_loc("context")] = context_list
    partial_df = df.iloc[sorted_idx]
    partial_pred = predict_from_df(partial_df, model_path)
    print(sorted_idx)
    prob[sorted_idx] = np.max([prob[sorted_idx], partial_pred], axis=0)
    return df, prob
    """

    # tfidfの値によってソートして新しいcontextにする
    context_list = sort_join_by_tfidf(df.iloc[sorted_idx], fts_result)
    df.iloc[sorted_idx, df.columns.get_loc("context")] = context_list
    partial_df = df.iloc[sorted_idx]
    partial_pred = predict_from_df(partial_df, model_path)
    print(sorted_idx)
...
def main(c: DictConfig) -> None:
    ...
    data2, pred2 = modify_prob_by_fts(
        df=data2, prob=pred2, fts_num_max=cfg.fts_num, model_path=cfg.model_path, fts_path=cfg.fts_path
    )
    out2 = predictions_to_map_output(pred2)
    true2 = data2["answer"].values
    map2 = map_k(true2, out2)
