...
def main(c: DictConfig) -> None:
    ...
    option_to_index = {option: idx for idx, option in enumerate("ABCDEFGHI")}
    index_to_option = {v: k for k, v in option_to_index.items()}

    def preprocess_train(example):
        first_sentence = [example["prompt_with_context"]] * 7
        # 不正解用として選択肢をランダムに二つ選んで組み合わせる
        random_options1 = random.sample(["A", "B", "C", "D", "E"], 2)
        sentence1 = " ".join([example[option] for option in random_options1])
        random_options2 = random.sample(["A", "B", "C", "D", "E"], 2)
        sentence2 = " ".join([example[option] for option in random_options2])
        second_sentences = [example[option] for option in "ABCDE"] + [sentence1, sentence2]
    ...
    tokenized_dataset_train = dataset_train.map(
        preprocess_train, remove_columns=["prompt_with_context", "prompt", "A", "B", "C", "D", "E", "answer"]
    )
