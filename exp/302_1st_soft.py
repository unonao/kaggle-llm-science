...
def main(c: DictConfig) -> None:
    ...
    def option_to_soft_label(option):
        soft_label = np.zeros(5)
        soft_label[ord(option) - ord("A")] = 1.0
        return soft_label

    df_valid["soft_label"] = df_valid["answer"].apply(option_to_soft_label).tolist()
    df_valid2["soft_label"] = df_valid2["answer"].apply(option_to_soft_label).tolist()
