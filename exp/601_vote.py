...
def main(c: DictConfig) -> None:
    ...
    pred2_map = predictions_to_map_output(pred2)
    pred3_map = predictions_to_map_output(pred3)
    true2 = data2["answer"].values
    true3 = data3["answer"].values
    map2 = map_k(true2, pred2_map)
    map3 = map_k(true3, pred3_map)
    print(f"map2:{map2}, map3:{map3}")

    # ある確率 p 以上と予測したものの数をカウント
    p = 0.95  # cfg.p
    for i in range(len(pred2_list)):
        pred2 += (pred2_list[i] >= p).astype(int)
        pred3 += (pred3_list[i] >= p).astype(int)
    pred2_map = predictions_to_map_output(pred2)
    pred3_map = predictions_to_map_output(pred3)
    map2 = map_k(true2, pred2_map)
    map3 = map_k(true3, pred3_map)
