...
def main(c: DictConfig) -> None:
    ...
    df_valid = pd.concat([pd.read_csv(path).head(num) for path, num in cfg.valid_data_paths.items()])
    ...
    # clean_text
    df_train["prompt_with_context"] = df_train["prompt_with_context"].apply(clean_text)
    df_valid["prompt_with_context"] = df_valid["prompt_with_context"].apply(clean_text)
    ...
    with utils.timer("valid"):
        ...
        result_dict = {
            "best_map@3": map_k(
                df_valid["answer"].head(200).to_numpy(), predictions_to_map_output(valid_pred[:200, :])
            ),
            "new_map@3": map_k(df_valid["answer"].to_numpy(), predictions_to_map_output(valid_pred)),
        }
        print(result_dict)
