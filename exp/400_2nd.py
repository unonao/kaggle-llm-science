...
def main(c: DictConfig) -> None:
    ...
    for data_name, data_dir in data_dir_dict.items():
        ...
        v2_df = pd.read_csv(os.path.join(data_dir, "data3_2nd.csv"))

        # 最初に学習に使わないカラムはセット
        if train_df is None:
            train_df = t_df[unuse_col]
            valid_df = v_df[unuse_col]
            valid2_df = v2_df[unuse_col]
        ...
        v2_df = v2_df.drop(unuse_col, axis=1)
        v2_df = v2_df.add_prefix(f"{data_name}_")

        # 学習データと検証データに結合
        train_df = pd.concat([train_df, t_df], axis=1)
        valid_df = pd.concat([valid_df, v_df], axis=1)
        valid2_df = pd.concat([valid2_df, v2_df], axis=1)

    print(train_df.shape, valid_df.shape, valid2_df.shape)
    print(train_df.head())
    ...
    lgb_eval2 = lgb.Dataset(valid2_df[use_cols], valid2_df["label"], reference=lgb_train)
    bst = lgb.train(
        OmegaConf.to_container(cfg.lgbm.params),
        lgb_train,
        valid_sets=[lgb_train, lgb_eval, lgb_eval2],
        valid_names=["train", "valid", "valid2"],
        num_boost_round=cfg.lgbm.num_boost_round,
        callbacks=[lgb.log_evaluation(cfg.lgbm.verbose_eval)],
    )
    print(
        f"best_itelation: {bst.best_iteration}, train: {bst.best_score['train']}, valid: {bst.best_score['valid']}, valid2: {bst.best_score['valid2']}"
    )
    ...
    pred = bst.predict(valid2_df[use_cols])
    valid2_df["pred"] = pred
    valid2_df = valid2_df.sort_values(["df_index", "pred"], ascending=[True, False])
    pred_df = valid2_df.groupby("df_index").agg({"option": list}).reset_index()
    pred_df["gt"] = valid2_df[valid2_df["label"] == 1].option.values
