...
def main(c: DictConfig) -> None:
    ...
    parquet_paths_base = [path for path in list(Path(cfg.parquet_dir).glob("*.parquet")) if "all" not in path.name]
    parquet_paths_base.sort()

    # 6分割して それぞれdbに書き込む
    import numpy as np

    parquet_paths_list = np.array_split(parquet_paths_base, 6)
    for pi, parquet_paths in enumerate(parquet_paths_list):
        print(pi, parquet_paths)
        db_path = preprocessed_path / f"fts{pi}.db"
        # db 作成
