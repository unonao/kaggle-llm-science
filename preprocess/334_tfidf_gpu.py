...
def retrieval(df_valid, modified_texts):
    modified_texts = pd.Series(modified_texts)
    corpus_df_valid = df_valid.apply(
        lambda row: clean_text_for_fts(
            f'{row["prompt"]}\n{row["prompt"]}\n{row["prompt"]}\n{row["A"]}\n{row["B"]}\n{row["C"]}\n{row["D"]}\n{row["E"]}'
        ),
        axis=1,
    )
    vectorizer1 = SkTfidfVectorizer(ngram_range=(1, 1), stop_words=stop_words)
    ...
    vectorizer = TfidfVectorizer(
        ...
        vocabulary=cudf.Series(vocab_df_valid),
    )
