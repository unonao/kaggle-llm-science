...
def compress_and_split_sections(
    sections: list[tuple[str, str]], title: str, max_sentence_length: int, max_sentence_num: int, filter_len=3
) -> list[str]:
    # セクション内を分割し、適切な長さになるように結合する

    document_sentences = []
    for header, content in sections:
        document = f"{header or 'No header'}: {content}" + "\n"
        section_sentences = []
        try:
    ...
    for path in glob.glob(f"{cfg.wiki_dir}/*.parquet"):
        ...
        for i, row in tqdm(df.iterrows(), total=len(df)):
            secs = compress_and_split_sections(
                extract_sections(row["text"]), row["title"], cfg.max_sentence_length, cfg.max_sentence_num
            )
            sections += secs
            ids += [row["id"]] * len(secs)
